...
public class accountInsertDialog extends javax.swing.JPanel{
    ...
    List<String> roles = roleDAO.getInstance().getAllRoleNames(); // Gọi phương thức mới
    cbxRole.removeAllItems(); // Xóa các mục cũ nếu có
    for (String role : roles) {
        cbxRole.addItem(role); // Thêm vào ComboBox
    }
}
    ...
    public boolean isValidInput(){
        if(txtAccountName.getText().isEmpty()){
            ...
            lblErrorAccountName.setText("Vui lòng nhập tên người dùng");
            return false;
        }
        else {
        ...
        if(cbxRole.getSelectedItem().equals("Chọn vai trò")){
            ...
            lblErrorRole.setText("Vui lòng chọn vai trò");
            return false;
        }
        else {
    ...
    public boolean isValidEmail(String email) {
        ...
        if (accountDAO.getInstance().selectByEmail(email) != null) {
            txtEmail.putClientProperty("JComponent.outline", "warning");
            lblErrorEmail.setText("Email đã tồn tại !");
            lblErrorEmail.setForeground(Color.ORANGE);
            return false;
        } else {
            txtEmail.putClientProperty("JComponent.outline", "null");
    ...
    if (isValidInput()) {
        ...
        // Kiểm tra nếu có chọn sinh viên
        if (cbxStudent.getSelectedItem() != null) {
            student = getIDByStudentName(cbxStudent.getSelectedItem().toString(), studentList);
        }

        // Kiểm tra nếu có chọn giảng viên
        if (cbxLecturer.getSelectedItem() != null) {
            lecturer = getIDByLecturerName(cbxLecturer.getSelectedItem().toString(), lecturerList);
        }
        ...
        accountModel am = new accountModel(name, email, user, password, role, student, lecturer);
